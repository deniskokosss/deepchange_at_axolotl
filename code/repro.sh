...
if [ $SKIP_EMBEDING != 0 ]; then
    ...
    if [ ! -e glossreader_embs_p2.tar.gz ]; then
        wget https://zenodo.org/records/13387855/files/glossreader_embs_p2.tar.gz
        tar -xvzf glossreader_embs_p2.tar.gz -C ../data/embedings
    fi
else
    ...
    if [ ! -e "../data/models/GR_Ru/model.safetensors" ]; then
        cd $HOME/data/models
        mkdir -p GR_Ru
        wget https://zenodo.org/records/13388066/files/model.safetensors -O GR_Ru/model.safetensors
        cd -
    fi
    ...
    if [ ! -e "../data/embedings/GR_Ru.json" ]; then
        echo "Vectorizing GR Ru"
        python gr_vectorize.py \
            --model ../data/models/GR_Ru/model.safetensors \
            --out_file ../data/embedings/GR_Ru.json
    fi
fi
...
for dataset in  "../axolotl24_shared_task/data/finnish/axolotl.test.fi.gold.tsv" \
    ...
    # --------- WSD: GR Ru --------- 
    out_file="../data/predictions/wsd_preds/GR_Ru_$fname"
    echo $out_file

    python gr_wsd.py \
        --vectors_file ../data/embedings/GR_Ru.json \
        --dataset $dataset \
        --pred $out_file

    mkdir -p ../results/WSD_GR_Ru
    python ../axolotl24_shared_task/code/evaluation/scorer_track1.py \
        --gold $dataset \
        --pred $out_file \
        -o ../results/WSD_GR_Ru/$fname
    ...
    mkdir -p ../results/WSI_agglomerative
    python ../axolotl24_shared_task/code/evaluation/scorer_track1.py \
        --gold $dataset \
